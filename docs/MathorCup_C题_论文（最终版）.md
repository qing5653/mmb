...
#### 5.2.3.3 阈值选取依据对照（与题目参考阈值对齐）

题目给出的高风险参考阈值为“血脂指标异常且痰湿积分>=60”，或“血脂正常但痰湿积分>=80且活动能力<40”。本文采用“分值阈值分层 + 规则语义校对”的双层依据，二者对应关系如下：

1. 主判定依据（分层阈值）：三级风险层级由轨道A去血脂风险分值阈值确定（0.561733, 0.998873），用于保证分层不与标签定义同源。
2. 语义对照依据（临床规则）：将题目参考阈值作为分层后解释规则，在高风险组合中核验其是否出现与是否具备高阳性率。
3. 对照结果A（与题目第一条一致）：在[outputs/q2/q2_high_risk_core_combos.csv](../outputs/q2/q2_high_risk_core_combos.csv)中，“痰湿积分>=60 + TG>=1.7”组内阳性率为1.0000；且在[outputs/q2/q2_risk_predictions.csv](../outputs/q2/q2_risk_predictions.csv)中，“血脂异常且痰湿>=60”样本共110例，阳性率为1.0000，说明与题目第一条参考阈值高度一致。
4. 对照结果B（题目第二条的样本边界）：在本数据中痰湿积分最大值为65，故“痰湿>=80且活动<40”条件无样本直接落入，无法做同口径统计检验；因此本文使用“痰湿积分>=60 + 活动<40”（见核心组合表）作为相邻强风险模式进行解释，保持“痰湿偏颇叠加低活动”的机制一致性。

由此可见，本文“阈值选取依据”不是单一来源：层级判定采用数据驱动分值阈值，临床语义采用题目参考规则对照校验，既保证可复现性，也保证与赛题参考阈值口径对齐。
