...
本文围绕中老年人群高血脂风险识别与干预优化问题，构建“特征识别与机理解释—风险分层预警—个体化干预优化”的一体化建模框架。首先，基于相关性筛选、L1-Logistic与随机森林投票机制识别关键风险指标，并通过九体质Logistic模型解释体质贡献方向；其次，构建融合模型分值与临床规则的三级风险分层体系，并通过多随机种子、消融、校准与Bootstrap区间完成稳健性验证；最后，在预算、年龄与活动能力约束下建立6个月干预优化模型，输出个体最优策略与可推广匹配规律。结果表明，风险分层具有清晰区分度，干预方案在现实约束下具备可行性与稳定性。问题二中，全模型AUC顶格反映了标签-特征同源上限；在严格无同源对照（去核心血脂+去血脂异常计数）下，测试AUC为0.7972，提示模型在更保守口径下仍具判别能力。需要强调的是，本文模型更适用于“已病识别与分层管理”，不宜直接外推为“未病早筛”。本文实现了从风险认知到策略生成的闭环，为慢病管理提供量化依据。
...
3. 验证集PR-AUC：1.0000。
4. 测试集PR-AUC：1.0000。
5. 严格无同源对照（去核心血脂+去血脂异常计数）验证/测试AUC：0.8582/0.7972。
...
1. 置乱标签检验：随机打乱训练标签后重训模型，AUC应回落至约0.5附近。
2. 泄漏排查检验：排除所有直接标签派生字段后重训，验证性能应出现合理下降。
3. 时序/分组外推检验：按时间或子群切分进行外推验证，观察AUC与Brier是否保持同方向结论。
...
4. 严格无同源对照（同时去除核心血脂与血脂异常计数）后，测试AUC降至0.7972，测试Brier升至0.13850。
...
1. 当前“去除核心血脂”实验（`remove_core_lipids`）中，AUC仍为1.0000，但Brier从0.00231上升到0.00756，说明判别排序仍强而概率质量下降。
2. 该现象提示模型仍可能通过血脂异常计数等衍生变量保留同源信息，因此“去核心血脂”不等价于“完全去同源”。
3. 在新增“严格无同源”实验（`strict_no_homology`）中，测试AUC由1.0000下降到0.7972（降幅0.2028），验证集AUC为0.8582，说明去除同源信息后模型仍保留中等以上判别能力，但已不再出现顶格效应。
4. 因此本文将“同源边界”作为主结论前提：当前融合模型适用于已病识别与分层管理；而严格无同源口径可作为未病预警的更保守参考下界。
...
稳健性验证进一步表明：在10个随机种子下模型指标几乎无波动；消融实验中AUC因数据顶格效应保持1.0，但Brier对特征删减敏感，提示模型校准质量会下降。进一步在严格无同源对照下，测试AUC回落至0.7972，验证了“顶格AUC主要受同源结构影响”的判断。因此本研究采用“区分能力+AUC、概率质量+Brier、分层有效性、同源边界对照”四维联合评价模型好坏。
...
2. 问题二：通过多随机种子重复、特征消融、概率校准与Bootstrap区间检验，验证分层结论对样本划分与特征扰动具有稳定性，并通过严格无同源对照（测试AUC=0.7972）明确模型外推边界。
...
3. 问题二同源边界层面，全模型顶格AUC代表诊断近端上限；严格无同源对照AUC=0.7972可作为更保守的未病预警参考。
...
2. 问题2务必并列写“同源上限AUC”与“严格无同源AUC=0.7972”，避免口径冲突。
